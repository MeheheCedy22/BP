...
#let primary-color = rgb("#00a9e0")
#let secondary-color = rgb("#016180")
#let tertiary-color = rgb("#2563eb")

#let middle-color = rgb("#0185b0")
#let special-color = rgb("#016180")

              // primary: rgb("#1e3a8a"),
              // primary: rgb("#00a9e0"), // fiit farba
              // primary-dark: rgb("#016180"),
  ...
  info.authors = {
    ...
    authors.map(author => if author.name == info.author {
      if "affiliation-id" in author {
        ...
        (name: author.name, affiliation-id: author.affiliation-id)
      } else {
  ...
  let body = {
    ...
    if info.affiliations != none {
      ...
      for affiliation in info.affiliations {
        ...
        text(size: 0.8em, affiliation.name)
        linebreak()
      }
    }
    // date
...
///
/// - `align` is the alignment of the content. Default is `horizon + center`.
#let clear-slide(align: left, body) = touying-slide-wrapper(self => {
  self = utils.merge-dicts(
    self,
    config-common(freeze-slide-counter: true),
    config-page(
      ...
      header: self => if self.store.title != none {
        block(
          ...
          place(left + horizon, text(fill: self.colors.neutral-lightest, weight: "bold", size: 0.8em, utils.call-or-display(self, self.store.title)), dx: 0.8em, dy: 0.3em),
        )
      },
      footer: none,
    ),
  )
  set text(fill: self.colors.neutral-darkest, size: 1.5em)
  touying-slide(self: self, _typst-builtin-align(align, body))
})
  ...
  show: touying-slides.with(
    ...
    config-methods(
      init: (self: none, body) => {
        ...
        show figure.caption: c => {
          text(fill: self.colors.neutral-darkest, weight: "regular")[
          #{
            smallcaps[#c.supplement]
          }]
          c.separator
          c.body
        }
    ...
    config-store(
        ...
        context {
          ...
          grid(
            ...
            block(
              ...
              fill: gradient.linear(self.colors.primary, middle-color),
            ...
            block(
              ...
              height: 0.8em,            
              // fill: self.colors.secondary,
              fill: gradient.linear(middle-color, self.colors.secondary),
            )
          )
        }
      },
      ...
      footer: self => {
        ...
        // if (context utils.slide-counter.display()) > "10" {
